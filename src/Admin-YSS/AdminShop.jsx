...
import { X, Trash2, Edit2, Plus, Search, Package, ArrowUpDown, Loader2 } from "lucide-react"
  ...
  const handleImageUpload = async (e, imageType) => {
    ...
    try {
      const uploadPromises = Array.from(files).map(async (file) => {
        ...
        const response = await axios.post("https://api.cloudinary.com/v1_1/dm97yk6vr/image/upload", formData)
  ...
  const handleSave = async () => {
    ...
    try {
      if (currentProduct.id) {
        ...
        setProducts((prev) =>
          prev.map((product) => (product.id === currentProduct.id ? { ...currentProduct } : product)),
        )
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      ...
      <div className="mb-6 flex items-center justify-between">
        ...
        <button onClick={fetchProducts} className="text-sm flex items-center text-gray-600 hover:text-black">
            ...
            >
                ...
                {product.image && product.image.length > 0 ? (
                  <>
                    ...
                    {product.secondaryImages && product.secondaryImages.length > 0 && (
                      <img
                        src={product.secondaryImages[0] || "/placeholder.svg"}
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {/* Color Tag */}
                  {product.color && (
                    <span className="text-xs px-2 py-1 bg-gray-100 text-gray-600 rounded-full">{product.color}</span>
                  )}
                </div>
                  ...
                  {Object.entries(product.stocks || {}).map(([size, count]) => {
                    ...
                    if (count === 0) {
                      ...
                      return (
                        ...
                        >
                          <span className="uppercase">{sizeMap[size]}</span>
                          <span className="font-bold">0</span>
                        </div>
                      )
                    } else if (count < 10) {
                      stockColor = "bg-red-50 text-red-600"
                    } else {
                      stockColor = "bg-green-50 text-green-600"
                    }
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    {/* Secondary Images */}
                    <div className="mb-3">
                      <label className="block text-sm font-medium text-gray-700 mb-1">Secondary Images (Back)</label>
                      <div className="flex items-center">
                        <input
                          type="file"
                          multiple
      ...
      {notification.show && (
        <div
          className={`fixed bottom-4 right-4 bg-${notification.type === "error" ? "red" : "green"}-500 text-white py-2 px-4 rounded-lg shadow-lg`}
        >
